...
def find_layers(arch_and_hp):
    ...
    if num_layers < max_layers:
        for i in range(max_layers - num_layers):
            layer_and_position.append([2,2,-1,-1])
...
if __name__ == '__main__':
    ...
    #preserve number of features
    # max=0
    # idx=-999
    # for i in range(len(test_init_params_std)):
    #     if max < np.shape(test_init_params_std[i])[0]:
    #         max = np.shape(test_init_params_std[i])[0]
    #         idx = i
    # max_row_size = max

    test_init_params_mu  = equal_len_initParams(test_init_params_mu, max_row_size)
    test_init_params_std = equal_len_initParams(test_init_params_std, max_row_size)
    test_init_params_l2  = equal_len_initParams(test_init_params_l2, max_row_size)
    ...
    #same thing but now for test data************************************************************
    #now we need to convert the arch_and_hp column into a numerical format
    #make into 2D array by formatting the arch_and_hp of all rows
    arch_and_hp = data_test.iloc[0,0]
    layer_and_position = find_layers(arch_and_hp)[:,:2].flatten()

    for i in range(1, len(data_test.iloc[:,0])):
        arch_and_hp = data_test.iloc[i,0]
        layer_and_position_temp = find_layers(arch_and_hp)[:,:2].flatten()
        layer_and_position = np.vstack((layer_and_position,\
                                       layer_and_position_temp))
    ...
    #layer num corresponds to layer type
    for i in range(layer_and_position.shape[1]):
        data_test['layer_type_{}'.format(i)] = layer_and_position[:,i]
        data_test['layer_num_{}'.format(i)]  = layer_and_position[:,i]

    #drop arch_and_hp column
    data_test = data_test.drop(columns=['arch_and_hp'])

    #there are no lables for the test data duh
    # #now get the labels
    # data_test_labels = pd.DataFrame()
    # data_test_labels['train_error'] = data_test['train_error']
    # data_test_labels['val_error']   = data_test['val_error']
    #
    # #drop the labels from data_train
    # data_test = data_test.drop(columns=['train_error', 'val_error'])
    ...
    # s=0
    # f=10
    # print(list(data_train.keys()[s:f]), list(data_test.keys())[s:f])

    data_train = data_train.drop(columns=['val_loss', 'train_loss'])

    #save into CSV file
    data_train.to_csv('../data/cleaned_training_data_try.csv')
    data_train_labels.to_csv('../data/cleaned_training_data_labels_try.csv')

    data_test.to_csv('../data/cleaned_testing_data_try.csv')
